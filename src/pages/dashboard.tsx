...
import { getFriendRequests } from 'apiSdk/friend-requests';
import { FriendRequestInterface } from 'interfaces/friend-request';

import { PetListPage as ShowCaseList } from 'pages/pets';
...
function DashboardPage() {
  ...
  const friendRequestStat = useSWR<PaginatedInterface<FriendRequestInterface>>(
    () => `/friend-requests`,
    () => getFriendRequests({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: friendRequestStat?.data?.totalCount,
      isLoading: friendRequestStat.isLoading,
    },
  ];
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`pets`} passHref legacyBehavior>
                  <Button
